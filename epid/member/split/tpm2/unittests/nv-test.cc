...
TEST(NvTest, NvWriteFailsGivenOverflow) {
  ...
  MembershipCredential const data[2] = {0};
  EXPECT_EQ(kEpidBadArgErr,
            Tpm2NvWrite(tpm, nv_index, sizeof(data[0]), 1, &data[0]));
  EXPECT_EQ(kEpidBadArgErr,
            Tpm2NvWrite(tpm, nv_index, sizeof(data[0]) + 1, 1, &data[0]));
  EXPECT_EQ(
      kEpidBadArgErr,
      Tpm2NvWrite(tpm, nv_index, 1, sizeof(MembershipCredential), &data[0]));
  THROW_ON_EPIDERR(Tpm2NvUndefineSpace(tpm, nv_index));
}
...
TEST(NvTest, NvReadFailsGivenOverflow) {
  ...
  MembershipCredential data[2] = {0};
  EXPECT_EQ(kEpidBadArgErr,
            Tpm2NvRead(tpm, nv_index, sizeof(data[0]), 1, &data[0]));
  EXPECT_EQ(kEpidBadArgErr,
            Tpm2NvRead(tpm, nv_index, sizeof(data[0]) + 1, 0, &data[0]));
  EXPECT_EQ(kEpidBadArgErr, Tpm2NvRead(tpm, nv_index, 1,
                                       sizeof(MembershipCredential), &data[0]));
  THROW_ON_EPIDERR(Tpm2NvUndefineSpace(tpm, nv_index));
}
