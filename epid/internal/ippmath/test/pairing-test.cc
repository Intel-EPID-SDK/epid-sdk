...
TEST_F(PairingTest, DISABLED_NewFailsGivenInvalidG1) {
  PairingState* ps = nullptr;
  EXPECT_EQ(kEpidBadArgErr,
            NewPairingState(this->params->G2, this->params->G2,
                            this->params->GT, &this->t_str, true, &ps));
  DeletePairingState(&ps);
}

// test that new checks that G2 is valid
TEST_F(PairingTest, DISABLED_NewFailsGivenInvalidG2) {
  PairingState* ps = nullptr;
  EXPECT_EQ(kEpidBadArgErr,
            NewPairingState(this->params->G1, this->params->G1,
                            this->params->GT, &this->t_str, true, &ps));
  DeletePairingState(&ps);
}
