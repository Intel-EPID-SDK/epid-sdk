...
#include "1.1/context.h"
}

#include "testhelper/1.1/verifier_wrapper-testhelper.h"
#include "testhelper/errors-testhelper.h"
#include "1.1/verifier-testhelper.h"
...
TEST_F(Epid11VerifierTest, WritePrecompFailsGivenNullPointer) {
  ...
  EXPECT_EQ(kEpidBadCtxErr, Epid11VerifierWritePrecomp(nullptr, &precomp));
  EXPECT_EQ(kEpidBadPrecompErr, Epid11VerifierWritePrecomp(ctx, nullptr));
}
...
TEST_F(Epid11VerifierTest, SetPrivRlFailsGivenZeroSize) {
  ...
  Epid11PrivRl prl = {0};
  prl.gid = this->kPubKeyStr.gid;
  EXPECT_EQ(kEpidBadPrivRlErr, Epid11VerifierSetPrivRl(verifier, &prl, 0));
}
...
TEST_F(Epid11VerifierTest, SetPrivRlFailsGivenN1TooBigForSize) {
  ...
  EXPECT_EQ(
      kEpidBadPrivRlErr,
      Epid11VerifierSetPrivRl(verifier, &prl, sizeof(prl) - sizeof(prl.f)));
}
...
TEST_F(Epid11VerifierTest, SetSigRlFailsGivenZeroSize) {
  ...
  Epid11SigRl* empty_sig_rl = (Epid11SigRl*)this->kEmptySigRl.data();
  EXPECT_EQ(kEpidBadSigRlErr,
            Epid11VerifierSetSigRl(verifier, empty_sig_rl, 0));
}
...
TEST_F(Epid11VerifierTest, SetSigRlFailsGivenN2TooSmallForSize) {
  ...
  std::vector<uint8_t> empty_sig_rl_buf(this->kEmptySigRl);
  Epid11SigRl* empty_sig_rl = (Epid11SigRl*)empty_sig_rl_buf.data();
  EXPECT_EQ(kEpidBadSigRlErr, Epid11VerifierSetSigRl(verifier, empty_sig_rl,
                                                     sizeof(*empty_sig_rl)));
}
...
TEST_F(Epid11VerifierTest, SetGroupRlFailsGivenSizeZero) {
  ...
  std::vector<uint8_t> group_rl(this->kGroupRl3GidBuf);
  Epid11GroupRl* grl = (Epid11GroupRl*)group_rl.data();
  EXPECT_EQ(kEpidBadGroupRlErr, Epid11VerifierSetGroupRl(verifier, grl, 0));
}
...
TEST_F(Epid11VerifierTest, SetBasenameFailsGivenNullContext) {
  ...
  EXPECT_EQ(kEpidBadCtxErr, Epid11VerifierSetBasename(nullptr, basename.data(),
                                                      basename.size()));
}
