...
#include "ecgroup-internal.h"
#include "finitefield-internal.h"
#include "pairing-internal.h"
...
EpidStatus Pairing(PairingState* ps, EcPoint const* a, EcPoint const* b,
  ...
  // check parameters
  if (!ps || !ps->Fq || !ps->Fq2 || !ps->ff || !ps->ff->ipp_ff ||
      !ps->Fq->ipp_ff || !ps->Fq2->ipp_ff || !ps->t || !ps->t->ipp_bn ||
      !ps->ga || !ps->ga->ipp_ec || !ps->gb || !ps->gb->ipp_ec) {
    return kEpidBadArgErr;
  }
  if (!d || !d->ipp_ff_elem) {
    return kEpidBadArgErr;
...
static EpidStatus FinalExp(PairingState* ps, FfElement* d, FfElement const* h) {
  ...
  // Check parameters
  if (!ps || !ps->ff || !ps->ff->ipp_ff || !ps->t || !ps->t->ipp_bn) {
    return kEpidBadArgErr;
  }
  if (!d || !d->ipp_ff_elem) {
    return kEpidBadArgErr;
...
static EpidStatus FrobeniusOp(PairingState* ps, FfElement* d_out,
  ...
  // check parameters
  if (!ps || !ps->ff || !ps->Fq2 || !ps->ff->ipp_ff || !ps->Fq2->ipp_ff) {
    return kEpidBadArgErr;
  }
  if (!d_out || !d_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!a || !a->ipp_ff_elem) {
    return kEpidBadArgErr;
...
static EpidStatus Line(FiniteField* gt, FfElement* f, FfElement* x_out,
  ...
  if (!x_out || !x_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!y_out || !y_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!z_out || !z_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!z2_out || !z2_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!px || !px->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!py || !py->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!x || !x->ipp_ff_elem) {
    return kEpidBadArgErr;
  ...
  if (!z2 || !z2->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!qx || !qx->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!qy || !qy->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!gt || !gt->ipp_ff) {
    return kEpidBadArgErr;
...
static EpidStatus MulXiFast(FfElement* e, FfElement const* a,
  ...
  if (!ps || !ps->Fq || !ps->Fq2 || !ps->Fq->ipp_ff || !ps->Fq2->ipp_ff) {
    return kEpidBadArgErr;
  }
...
static EpidStatus MulV(FfElement* e, FfElement* a, PairingState* ps) {
  ...
  if (!ps || !ps->Fq2 || !ps->Fq6 || !ps->Fq2->ipp_ff || !ps->Fq6->ipp_ff) {
    return kEpidBadArgErr;
  }
...
static EpidStatus Fq6MulGFpE2(FfElement* e, FfElement* a, FfElement* b0,
  ...
  if (!b0 || !b0->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!b1 || !b1->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!ps || !ps->Fq2 || !ps->Fq6 || !ps->Fq2->ipp_ff || !ps->Fq6->ipp_ff) {
    return kEpidBadArgErr;
...
static EpidStatus SquareForFq4(PairingState* ps, FfElement* e0, FfElement* e1,
  ...
  // check parameters
  if (!e0 || !e0->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!e1 || !e1->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!a0 || !a0->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!a1 || !a1->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!ps || !ps->Fq2 || !ps->Fq6 || !ps->ff || !ps->ff->ipp_ff ||
      !ps->Fq2->ipp_ff || !ps->Fq6->ipp_ff) {
    return kEpidBadArgErr;
  }
...
static EpidStatus SquareCyclotomic(PairingState* ps, FfElement* e_out,
  ...
  // check parameters
  if (!e_out || !e_out->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!a_in || !a_in->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (!ps || !ps->Fq || !ps->Fq2 || !ps->Fq6 || !ps->ff || !ps->ff->ipp_ff ||
