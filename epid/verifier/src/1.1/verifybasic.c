/*############################################################################
  ...
  ############################################################################*/
/// Epid11VerifyBasicSig implementation.
/*! \file */
#include <stdio.h>
#include <string.h>
...
EpidStatus Epid11VerifyBasicSig(Epid11VerifierCtx const* ctx,
  ...
  if (!ctx || !ctx->epid11_params || !ctx->pub_key) {
    return kEpidBadCtxErr;
  }
  if (!sig) {
    return kEpidBadSignatureErr;
  }
  if ((!msg && (0 != msg_len)) || msg_len > UINT_MAX) {
  ...
  do {
    ...
    if (kEpidNoErr != res) {
      if (ctx->basename_len == 0 && EPID_IS_BADARG_ERROR(res)) {
        res = kEpidSigInvalid;
      }
      break;
    }
