...
void* NewBufferFromFile(char const* filename, size_t* size) {
  void* buffer = NULL;
...
int ReadBufferFromFileLoud(char const* filename, void* buffer, size_t size,
  ...
  do {
    ...
    fgetc(fp);  // read one more byte, hopefully setting feof
    if (bytes_read != size || !feof(fp)) {
      log_error("%s: %s has invalid format", filename, buf_name);
      result = -1;
      break;
    }
  } while (0);
...
void* NewBufferFromFileLoud(const char* filename, size_t* size,
                            char const* buf_name) {
  void* buffer = NULL;
  ...
  do {
    ...
    if (buffer) {
      if (0 != ReadBufferFromFileLoud(filename, buffer, len, buf_name)) {
        free(buffer);
        buffer = NULL;
        break;
      }
