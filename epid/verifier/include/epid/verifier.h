...
/*! \file */
#ifndef EPID_VERIFIER_INCLUDE_EPID_VERIFIER_H_
#define EPID_VERIFIER_INCLUDE_EPID_VERIFIER_H_
...
/*!
  ...
  To use this module, include the header epid/verifier.h.
...
/*!
 ...
 \deprecated This API has been superseded by ::EpidVerifierGetSize and
 ::EpidVerifierInit.
...
/// Compute the size in bytes required for a verifier context
/*!
 * \param[out] context_size
 *
 * Number of bytes required for a ::VerifierCtx buffer
 *
 * \returns ::EpidStatus
 *
 * \see EpidVerifierInit
 */
 ...
 *
 * \param[in] grp_rl
 * The group based revocation list. If NULL, this is interpreted as an
 ...
 *
 * \param[out] ctx
 * An existing buffer that will be used as an ::VerifierCtx.
 *
 * \attention
 ...
 *
 * \attention
 * It is the responsibility of the caller to free the memory pointed to by
 * grp_rl after the verifier is no longer using it.
 *
 ...
 * \retval ::kEpidNoErr
 *
 * \retval ::kEpidBadCtxErr
 *
 * \retval ::kEpidBadGroupRlErr
 *
 * \see EpidVerifierGetSize
 */
...
/*!
 ...
 \deprecated This API has been superseded by ::EpidVerifierDeinit.
...
/// De-initialize an existing verifier context buffer.
/*!
 * Must be called to safely release a verifier context initialized using
 * ::EpidVerifierInit.
 *
 * \param[in,out] ctx
 * The verifier context. Can be NULL.
...
/// Sets the group against which signatures will be verified
/*!
 *
 *
 * \param[in,out] ctx
 ...
 *
 * \param[in] priv_rl
 * The private key based revocation list. If NULL, this is interpreted as an
 * empty revocation list.
 * \param[in] priv_rl_size
 * The size of the private key based revocation list in bytes.
 * \param[in] sig_rl
 * The signature based revocation list. If NULL, this is interpreted as an empty
 * revocation list.
 * \param[in] sig_rl_size
 * The size of the signature based revocation list in bytes.
 *
 * \attention
 * The memory pointed to by priv_rl and sig_rl is accessed directly by the
 * verifier until a new list is set or the verifier is destroyed. Do not modify
 * the contents of this memory. The behavior of subsequent operations that rely
 * on the revocation list is undefined if the memory is modified.
 *
 * \attention
 * It is the responsibility of the caller to free the memory pointed to by
 * priv_rl and sig_rl after the verifier is no longer using it.
 *
 * \retval ::kEpidNoErr
 * \retval ::kEpidBadCtxErr
 * \retval ::kEpidBadGroupPubKeyErr
 * \retval ::kEpidSchemaNotSupportedErr
 * \retval ::kEpidHashAlgorithmNotSupported
 * \retval ::kEpidBadPrecompErr
 * \retval ::kEpidBadPrivRlErr
 * \retval ::kEpidBadSigRlErr
 *
 */
...
/*!
  ...
  \note You must set the group against which to verify the using
  ::EpidVerifierSetGroup before calling this function.
...
/*!
 ...
 A signature.
 \param[in] sig1_len
 The size of sig1 in bytes.
 \param[in] sig2
 A signature.
 \param[in] sig2_len
 The size of sig2 in bytes.
 ...
 \note
 The input signatures should be verified using EpidVerify() before
...
#endif  // EPID_VERIFIER_INCLUDE_EPID_VERIFIER_H_
