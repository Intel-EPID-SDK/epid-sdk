...
EpidStatus NewFiniteFieldViaBinomialExtension(FiniteField const* ground_field,
  ...
  if (!ground_field || !ground_field->ipp_ff) {
    return kEpidBadArgErr;
  }
  if (!ground_element || !ground_element->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
  if (degree < 2) {
    return kEpidBadArgErr;
...
void ZeroizeFfElement(FiniteField const* ff, FfElement** ff_elem) {
  if (ff_elem) {
    if (*ff_elem) {
      if ((*ff_elem)->ipp_ff_elem) {
        unsigned int ctxsize = 0;
        // Determine the memory requirement for finite field element context
        if (ippsGFpElementGetSize(ff->ipp_ff, (int*)&ctxsize) == ippStsNoErr) {
          EpidZeroMemory((*ff_elem)->ipp_ff_elem, ctxsize);
        }
      }
    }
  }
...
EpidStatus ReadFfElement(FiniteField* ff, ConstOctStr ff_elem_str,
  ...
  if (!ff_elem || !ff_elem->ipp_ff_elem) {
    return kEpidBadArgErr;
  }
...
EpidStatus GetFiniteFieldPrime(FiniteField* ff, BigNum** bn) {
  if (!ff || !ff->ipp_ff || ff->basic_degree != 1 || ff->ground_degree != 1) {
    return kEpidBadArgErr;
  }
  if (!bn) {
    return kEpidBadArgErr;
...
EpidStatus FfAdd(FiniteField* ff, FfElement const* a, FfElement const* b,
  ...
  if (ff->element_len != b->element_len) {
    return kEpidBadArgErr;
  }
  if (ff->element_len != r->element_len) {
    return kEpidBadArgErr;
...
EpidStatus FfIsUnity(FiniteField* ff, FfElement const* a, bool* is_unity) {
  IppStatus sts = ippStsNoErr;
  int ipp_result = IPP_IS_NE;
  // Check required parameters
  if (!ff || !ff->ipp_ff) {
  ...
  // Check if the element is unity
  sts = ippsGFpIsUnityElement(a->ipp_ff_elem, &ipp_result, ff->ipp_ff);
  // Check return codes
  if (ippStsNoErr != sts) {
    if (ippStsContextMatchErr == sts)
      return kEpidBadArgErr;
