...
############################################################################*/
/// Intel Intel(R) EPID 1.1 Verifier context implementation.
/*! \file */
...
EpidStatus Epid11VerifierCreate(Epid11GroupPubKey const* pub_key,
  ...
  do {
    ...
    if (EPID_IS_BADARG_ERROR(result)) {
      result = kEpidBadGroupPubKeyErr;
    }
    BREAK_ON_EPID_ERROR(result);
...
EpidStatus Epid11VerifierWritePrecomp(Epid11VerifierCtx const* ctx,
  ...
  if (!ctx || !ctx->e12 || !ctx->e22 || !ctx->e2w || !ctx->epid11_params ||
      !(ctx->epid11_params->GT) || !ctx->pub_key) {
    return kEpidBadCtxErr;
  }
  if (!precomp) {
...
EpidStatus Epid11VerifierSetPrivRl(Epid11VerifierCtx* ctx,
  ...
  // priv_rl is checked for NULL inside the IsEpid11PrivRlValid function
...
EpidStatus Epid11VerifierSetSigRl(Epid11VerifierCtx* ctx,
  ...
  // sig_rl is checked for NULL inside the Epid11IsSigRlValid function
...
EpidStatus Epid11VerifierSetGroupRl(Epid11VerifierCtx* ctx,
  ...
  // grp_rl is checked for NULL inside the Epid11IsGroupRlValid function
...
static EpidStatus ReadPrecomputation(Epid11VerifierPrecomp const* precomp_str,
  ...
  if (!ctx || !ctx->epid11_params || !ctx->epid11_params->GT || !ctx->e12 ||
      !ctx->e22 || !ctx->e2w) {
    return kEpidBadArgErr;
  }
  if (!ctx->pub_key) {
