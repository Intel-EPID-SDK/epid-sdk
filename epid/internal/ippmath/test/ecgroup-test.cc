...
extern const std::vector<uint8_t> kValidG1Elem;
extern const std::vector<uint8_t> kValidG2Elem;
extern const std::vector<uint8_t> kG1ElemWithXNotInFq;
extern const std::vector<uint8_t> kG1ElemWithYNotInFq;
extern const std::vector<uint8_t> kG1ElemWithValidXYNotOnCurve;
extern const std::vector<uint8_t> kG2ElemWithXNotInFq2;
extern const std::vector<uint8_t> kG2ElemWithYNotInFq2;
extern const std::vector<uint8_t> kG2ElemWithValidXYNotOnCurve;
...
#include "common/1.1/epid11params_tate.inc"
    };
...
// Test disabled because the current implementation selects the root
// deterministically rather than selecting a root at random.
TEST_F(EcGroupTest, DISABLED_GetRandomSelectsMinRootRandomly) {
  EcPointObj r(&this->efq);
  OneTimePad otp({0x25, 0xeb, 0x8c, 0x48, 0xff, 0x89, 0xcb, 0x85, 0x4f, 0xc0,
                  0x90, 0x81, 0xcc, 0x47, 0xed, 0xfc, 0x86, 0x19, 0xb2, 0x14,
                  0xfe, 0x65, 0x92, 0xd4, 0x8b, 0xfc, 0xea, 0x9c, 0x9d, 0x8e,
                  0x32, 0x44, 0xd7, 0xd7, 0xe9, 0xf1, 0xf7, 0xde, 0x60, 0x56,
                  0x8d, 0xe9, 0x89, 0x07, 0x3f, 0x3d, 0x16, 0x39, 0x00});

  G1ElemStr expected = {
      {0x2d, 0x26, 0x75, 0x86, 0x43, 0x5e, 0x8e, 0xda, 0x3d, 0xf3, 0x61,
       0x26, 0x9d, 0x84, 0x3d, 0xf6, 0x69, 0x90, 0x83, 0x3d, 0x81, 0xee,
       0xa6, 0x21, 0xf8, 0x24, 0xc1, 0x27, 0xd7, 0x85, 0x42, 0xd3},
      {0xae, 0x55, 0x90, 0xc9, 0x6d, 0x0a, 0xe6, 0xde, 0x2c, 0x52, 0x3d,
       0x9a, 0x49, 0x27, 0x88, 0x59, 0x02, 0x90, 0xb5, 0xbc, 0x61, 0xce,
       0xce, 0x5d, 0x59, 0x67, 0xc5, 0x20, 0x1b, 0x8d, 0xc1, 0x4b}};

  G1ElemStr actual = {0};

  EXPECT_EQ(kEpidNoErr, EcGetRandom(this->efq, OneTimePad::Generate, &otp, r));
  THROW_ON_EPIDERR(WriteEcPoint(this->efq, r, &actual, sizeof(actual)));
  EXPECT_EQ(expected, actual);
  EXPECT_EQ(385u, otp.BitsConsumed());
}

// Test disabled because the current implementation selects the root
// deterministically rather than selecting a root at random.
TEST_F(EcGroupTest, DISABLED_GetRandomSelectsMaxRootRandomly) {
  EcPointObj r(&this->efq);
  OneTimePad otp({0x25, 0xeb, 0x8c, 0x48, 0xff, 0x89, 0xcb, 0x85, 0x4f, 0xc0,
                  0x90, 0x81, 0xcc, 0x47, 0xed, 0xfc, 0x86, 0x19, 0xb2, 0x14,
                  0xfe, 0x65, 0x92, 0xd4, 0x8b, 0xfc, 0xea, 0x9c, 0x9d, 0x8e,
                  0x32, 0x44, 0xd7, 0xd7, 0xe9, 0xf1, 0xf7, 0xde, 0x60, 0x56,
                  0x8d, 0xe9, 0x89, 0x07, 0x3f, 0x3d, 0x16, 0x39, 0x01});

  G1ElemStr expected = {
      {0x2d, 0x26, 0x75, 0x86, 0x43, 0x5e, 0x8e, 0xda, 0x3d, 0xf3, 0x61,
       0x26, 0x9d, 0x84, 0x3d, 0xf6, 0x69, 0x90, 0x83, 0x3d, 0x81, 0xee,
       0xa6, 0x21, 0xf8, 0x24, 0xc1, 0x27, 0xd7, 0x85, 0x42, 0xd3},
      {0x51, 0xaa, 0x6f, 0x36, 0x92, 0xf2, 0x09, 0xef, 0x1a, 0x93, 0xb4,
       0xc4, 0xa5, 0x4a, 0x1c, 0x46, 0x0a, 0x4b, 0xb0, 0x3e, 0xb0, 0xc9,
       0x3c, 0x25, 0x79, 0xc1, 0x68, 0xbb, 0x93, 0x45, 0x6e, 0xc8}};
...
// Test disabled because the current implementation selects the root
// deterministically rather than selecting a root at random.
TEST_F(EcGroupTest, DISABLED_GetRandomSelectsRootRandomlyWhenFirstRootFails) {
  EcPointObj r(&this->efq);
  OneTimePad otp(
  ...
  G1ElemStr expected = {
      {0xeb, 0x42, 0x69, 0x88, 0xc4, 0xef, 0x71, 0x01, 0x95, 0xf3, 0x2c,
       0xb3, 0x07, 0xfd, 0x66, 0xd6, 0xf4, 0x2f, 0xdc, 0x1f, 0xb1, 0xd3,
       0x5d, 0x89, 0x59, 0x36, 0x6f, 0x56, 0x5f, 0x65, 0x19, 0x9e},
      {0xdd, 0x2f, 0x36, 0xb3, 0x98, 0xdd, 0x88, 0x9e, 0x93, 0x2c, 0x04,
       0x7d, 0x5c, 0xec, 0x3f, 0x73, 0x4e, 0xd6, 0x5e, 0xa4, 0x35, 0xe3,
       0x39, 0x40, 0x09, 0x0d, 0x0e, 0xd6, 0x2b, 0x91, 0xbe, 0x1e}};
  ...
  THROW_ON_EPIDERR(WriteEcPoint(this->efq, r, &actual, sizeof(actual)));
  EXPECT_EQ(expected, actual);
  EXPECT_EQ(1537u, otp.BitsConsumed());
}
///////////////////////////////////////////////////////////////////////
// EcInGroup
...
TEST_F(EcGroupTest, InGroupDetectsElementInG1) {
  EpidStatus sts = kEpidNoErr;
  bool in_group = false;
  G1ElemStr* pt_in_g1 = (G1ElemStr*)kValidG1Elem.data();
  sts = EcInGroup(this->efq, pt_in_g1, sizeof(*pt_in_g1), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_TRUE(in_group);
}

TEST_F(EcGroupTest, InGroupDetectsElementInG2) {
  EpidStatus sts = kEpidNoErr;
  bool in_group = false;
  G2ElemStr* pt_in_g2 = (G2ElemStr*)kValidG2Elem.data();
  sts = EcInGroup(this->efq2, pt_in_g2, sizeof(*pt_in_g2), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_TRUE(in_group);
}

TEST_F(EcGroupTest, InGroupDetectsElementNotInG1) {
  EpidStatus sts = kEpidNoErr;
  bool in_group = false;
  // x not in Fq
  G1ElemStr* x_not_in_fq = (G1ElemStr*)kG1ElemWithXNotInFq.data();
  sts = EcInGroup(this->efq, x_not_in_fq, sizeof(*x_not_in_fq), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
  // y not in Fq
  G1ElemStr* y_not_in_fq = (G1ElemStr*)kG1ElemWithYNotInFq.data();
  sts = EcInGroup(this->efq, y_not_in_fq, sizeof(*y_not_in_fq), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
  // x and y in Fq but not on curve y^2 = x^3 +ax + b
  G1ElemStr* xy_not_on_curve = (G1ElemStr*)kG1ElemWithValidXYNotOnCurve.data();
  sts = EcInGroup(this->efq, xy_not_on_curve, sizeof(*xy_not_on_curve),
                  &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
}

TEST_F(EcGroupTest, InGroupDetectsElementNotInG2) {
  EpidStatus sts = kEpidNoErr;
  bool in_group = false;
  // x not in Fq2
  G2ElemStr* x_not_in_fq2 = (G2ElemStr*)kG2ElemWithXNotInFq2.data();
  sts = EcInGroup(this->efq2, x_not_in_fq2, sizeof(*x_not_in_fq2), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
  // y not in Fq2
  G2ElemStr* y_not_in_fq2 = (G2ElemStr*)kG2ElemWithYNotInFq2.data();
  sts = EcInGroup(this->efq2, y_not_in_fq2, sizeof(*y_not_in_fq2), &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
  // x and y in Fq2 but not on curve y^2 = x^3 +ax + b
  G2ElemStr* xy_not_on_curve = (G2ElemStr*)kG2ElemWithValidXYNotOnCurve.data();
  sts = EcInGroup(this->efq2, xy_not_on_curve, sizeof(*xy_not_on_curve),
                  &in_group);
  EXPECT_EQ(kEpidNoErr, sts);
  EXPECT_FALSE(in_group);
}
...
const std::vector<uint8_t> kValidG1Elem = {
...
const std::vector<uint8_t> kValidG2Elem = {
...
const std::vector<uint8_t> kG1ElemWithXNotInFq = {
...
const std::vector<uint8_t> kG1ElemWithYNotInFq = {
    0x3d, 0x4b, 0x62, 0xf9, 0xf9, 0x1a, 0x0f, 0xb1,
    0x58, 0x7b, 0x0e, 0x42, 0x0c, 0xba, 0x94, 0x10,
...
const std::vector<uint8_t> kG1ElemWithValidXYNotOnCurve = {
    0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0xf0, 0xcd,
    0x46, 0xe5, 0xf2, 0x5e, 0xee, 0x71, 0xa4, 0x9f,
    ...
    0x0c, 0xdc, 0x65, 0xfb, 0x12, 0x98, 0x0a, 0x82,
    0xd3, 0x29, 0x2d, 0xdb, 0xae, 0xd3, 0x30, 0x1};

const std::vector<uint8_t> kG2ElemWithXNotInFq2 = {
    ...
    0xfa, 0x9c, 0xf1, 0xd7, 0x98, 0xfe, 0x28, 0x9e,
    0xc1, 0x81, 0xe7, 0x51, 0xce, 0x07, 0xbc, 0xc5};
const std::vector<uint8_t> kG2ElemWithYNotInFq2 = {
    0xb0, 0x63, 0x98, 0x76, 0x04, 0xcf, 0x85, 0xd9,
    0xad, 0xf2, 0x02, 0x0c, 0x13, 0x2c, 0x45, 0x10,
    ...
    0xfa, 0x9c, 0xf1, 0xd7, 0x98, 0xfe, 0x28, 0x9e,
    0xc1, 0x81, 0xe7, 0x51, 0xce, 0x07, 0xbc, 0xc5};
const std::vector<uint8_t> kG2ElemWithValidXYNotOnCurve = {
    0xb0, 0x63, 0x98, 0x76, 0x04, 0xcf, 0x85, 0xd9,
    0xad, 0xf2, 0x02, 0x0c, 0x13, 0x2c, 0x45, 0x10,

    0x5b, 0xcb, 0xa8, 0x76, 0xde, 0x1e, 0xe6, 0x92,
    0x06, 0x91, 0x49, 0xc2, 0x1a, 0xe8, 0x80, 0x91,
